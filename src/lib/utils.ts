...
export const groupInterviews = (interviews: Interview[]) => {
  ...
  return interviews.reduce((acc: any, interview: Interview) => {
    ...
    if (interview.status === "đạt yêu cầu") {
      acc.succeeded = [...(acc.succeeded || []), interview];
    } else if (interview.status === "không đạt") {
...
export const getMeetingStatus = (interview: Interview) => {
  ...
  if (
    ...
    interview.status === "thất bại" ||
    interview.status === "thành công"
  )
    return "hoàn thành";
  if (isWithinInterval(now, { start: interviewStartTime, end: endTime })) return "đang diễn ra";
  if (isBefore(now, interviewStartTime)) return "sắp diễn ra";
  return "hoàn thành";
};