...
function CommentDialog({ interviewId }: { interviewId: Id<"interviews"> }) {
  ...
  const handleSubmit = async () => {
    if (!comment.trim()) return toast.error("Hãy thêm nhận xét");
    ...
    try {
      ...
      toast.success("Nhận xét đã được lưu");
      setComment("");
      setRating("3");
      setIsOpen(false);
    } catch (error) {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      ...
      <DialogContent className="sm:max-w-[600px]">
        <DialogHeader>
          <DialogTitle>Nhận xét buổi phỏng vấn</DialogTitle>
        </DialogHeader>
          ...
          {existingComments.length > 0 && (
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                <h4 className="text-sm font-medium">Các nhận xét khác</h4>
                <Badge variant="outline">
                  {existingComments.length}Nhận xét{existingComments.length !== 1 ? "s" : ""}
                </Badge>
              </div>
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              <Textarea
                ...
                placeholder="Nhận xét chi tiết của bạn về ứng viên..."
                className="h-32"
              />
            </div>
          </div>
